...
@Component({
  selector: 'app-phone-upload',
  templateUrl: './phone-upload.component.html',
  styleUrls: ['./phone-upload.component.scss']
})
export class PhoneUploadComponent implements OnInit {
  ...
  uploadFile() {
    if (typeof (FileReader) !== 'undefined') {
      ...
      reader.onloadend = (e => {
        ...
        this.employeeService.uploadPhoneFile(this.file).subscribe({
          next: (result) => { },
          error: (err) => {
  ...
  download() {
    this.employeeService.downloadPhoneExcelFile().subscribe();
  }
